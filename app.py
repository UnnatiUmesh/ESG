...
def filter_on_date(df, start, end, date_col="DATE"):
    # Convert the column to datetime.date type for comparison
    df[date_col] = pd.to_datetime(df[date_col]).dt.date
    # Ensure start and end are also date types
...
def main(start_data, end_data):
    ...
    if company and company != "Select a Company":
        ...
        df_company = filter_company_data(df_company, esg_categories,
                                         start, end)
        esg_df = filter_on_date(esg_df, start, end, date_col="DATE")
        ...
        avg_esg = avg_esg.apply(pd.to_numeric, errors='coerce')
